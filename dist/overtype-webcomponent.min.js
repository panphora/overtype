...
${a}`:r;if(d){let k=t.value[t.selectionStart-1];t.selectionStart!==0&&k!=null&&!k.match(/\s/)&&(g=` ${g}`)}m=En(t,g,v,e.multiline);let b=t.selectionStart,w=t.selectionEnd,S=c&&c.length>0&&v.indexOf(c)>-1&&m.length>0;if(p){let k=Re(t);n=k.newlinesToAppend,i=k.newlinesToPrepend,g=n+o,v+=i}if(m.startsWith(g)&&m.endsWith(v)){let k=m.slice(g.length,m.length-v.length);if(h===f){let L=h-g.length;L=Math.max(L,b),L=Math.min(L,b+k.length),b=w=L}else w=b+k.length;return{text:k,selectionStart:b,selectionEnd:w}}else if(S)if(l&&l.length>0&&m.match(l)){v=v.replace(c,m);let k=g+v;return b=w=b+g.length,{text:k,selectionStart:b,selectionEnd:w}}else{let k=g+m+v;return b=b+g.length+m.length+v.indexOf(c),w=b+c.length,{text:k,selectionStart:b,selectionEnd:w}}else{let k=g+m+v;b=h+g.length,w=f+g.length;let L=m.match(/^\s*|\s*$/g);if(u&&L){let me=L[0]||"",Y=L[1]||"";k=me+g+m.trim()+v+Y,b+=me.length,w-=Y.length}return{text:k,selectionStart:b,selectionEnd:w}}}function it(t,e){let{prefix:n,suffix:i,surroundWithNewlines:o}=e,r=t.value.slice(t.selectionStart,t.selectionEnd),s=t.selectionStart,a=t.selectionEnd,c=r.split(`
