...
`.repeat(2-s)),{newlinesToAppend:a,newlinesToPrepend:l}}function be(t,e,n={}){let i=t.selectionStart,o=t.selectionEnd,r=i===o,s=t.value,a=i;for(;a>0&&s[a-1]!==`
`;)a--;if(r){let p=i;for(;p<s.length&&s[p]!==`
`;)p++;t.selectionStart=a,t.selectionEnd=p}else nt(t);let l=e(t);if(n.adjustSelection){let c=t.value.slice(t.selectionStart,t.selectionEnd).startsWith(n.prefix),d=n.adjustSelection(c,i,o,a);l.selectionStart=d.start,l.selectionEnd=d.end}else if(n.prefix){let c=t.value.slice(t.selectionStart,t.selectionEnd).startsWith(n.prefix);r?c?(l.selectionStart=Math.max(i-n.prefix.length,a),l.selectionEnd=l.selectionStart):(l.selectionStart=i+n.prefix.length,l.selectionEnd=l.selectionStart):c?(l.selectionStart=Math.max(i-n.prefix.length,a),l.selectionEnd=Math.max(o-n.prefix.length,a)):(l.selectionStart=i+n.prefix.length,l.selectionEnd=o+n.prefix.length)}return l}function xe(t,e){let n,i,{prefix:o,suffix:r,blockPrefix:s,blockSuffix:a,replaceNext:l,prefixSpace:p,scanFor:c,surroundWithNewlines:d,trimFirst:h}=e,u=t.selectionStart,f=t.selectionEnd,m=t.value.slice(t.selectionStart,t.selectionEnd),g=Xe(m)&&s&&s.length>0?`${s}
`:o,y=Xe(m)&&a&&a.length>0?`
${a}`:r;if(p){let k=t.value[t.selectionStart-1];t.selectionStart!==0&&k!=null&&!k.match(/\s/)&&(g=` ${g}`)}m=Ln(t,g,y,e.multiline);let w=t.selectionStart,b=t.selectionEnd,L=l&&l.length>0&&y.indexOf(l)>-1&&m.length>0;if(d){let k=$e(t);n=k.newlinesToAppend,i=k.newlinesToPrepend,g=n+o,y+=i}if(m.startsWith(g)&&m.endsWith(y)){let k=m.slice(g.length,m.length-y.length);if(u===f){let E=u-g.length;E=Math.max(E,w),E=Math.min(E,w+k.length),w=b=E}else b=w+k.length;return{text:k,selectionStart:w,selectionEnd:b}}else if(L)if(c&&c.length>0&&m.match(c)){y=y.replace(l,m);let k=g+y;return w=b=w+g.length,{text:k,selectionStart:w,selectionEnd:b}}else{let k=g+m+y;return w=w+g.length+m.length+y.indexOf(l),b=w+l.length,{text:k,selectionStart:w,selectionEnd:b}}else{let k=g+m+y;w=u+g.length,b=f+g.length;let E=m.match(/^\s*|\s*$/g);if(h&&E){let me=E[0]||"",Y=E[1]||"";k=me+g+m.trim()+y+Y,w+=me.length,b-=Y.length}return{text:k,selectionStart:w,selectionEnd:b}}}function it(t,e){let{prefix:n,suffix:i,surroundWithNewlines:o}=e,r=t.value.slice(t.selectionStart,t.selectionEnd),s=t.selectionStart,a=t.selectionEnd,l=r.split(`
...
`),o){let{newlinesToAppend:c,newlinesToPrepend:d}=$e(t);s+=c.length,a=s+r.length,r=c+r+d}return{text:r,selectionStart:s,selectionEnd:a}}function Ye(t){let e=t.split(`
...
`).length-1}handleInput(e){this.updatePreview()}handleKeydown(e){if(e.key==="Tab"){let i=this.textarea.selectionStart,o=this.textarea.selectionEnd,r=this.textarea.value;if(e.shiftKey&&i===o)return;if(e.preventDefault(),i!==o&&e.shiftKey){let s=r.substring(0,i),a=r.substring(i,o),l=r.substring(o),c=a.split(`
