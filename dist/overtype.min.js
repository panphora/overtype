...
`).length,"lines")}catch(p){_=!1,o&&console.log("execCommand threw error:",p)}t.contentEditable="false"}if(o&&(console.log("canInsertText before:",_),console.log("execCommand result:",_)),_){let p=a+e+c,h=t.value;o&&(console.log("Expected length:",p.length),console.log("Actual length:",h.length)),h!==p&&o&&(console.log("execCommand changed the value but not as expected"),console.log("Expected:",JSON.stringify(p.slice(0,100))),console.log("Actual:",JSON.stringify(h.slice(0,100))))}if(!_)if(o&&console.log("Using manual insertion"),t.value===d){o&&console.log("Value unchanged, doing manual replacement");try{document.execCommand("ms-beginUndoUnit")}catch(p){}t.value=a+e+c;try{document.execCommand("ms-endUndoUnit")}catch(p){}t.dispatchEvent(new CustomEvent("input",{bubbles:!0,cancelable:!0}))}else o&&console.log("Value was changed by execCommand, skipping manual insertion");o&&console.log("Setting selection range:",n,i),n!=null&&i!=null?t.setSelectionRange(n,i):t.setSelectionRange(r,t.selectionEnd),o&&(console.log("Final value length:",t.value.length),console.groupEnd())}function Xe(t){return t.trim().split(`
...
`),o){let{newlinesToAppend:l,newlinesToPrepend:p}=$e(t);s+=l.length,a=s+r.length,r=l+r+p}return{text:r,selectionStart:s,selectionEnd:a}}function Ye(t){let e=t.split(`
