...
${a}`:r;if(c){let x=o.value[o.selectionStart-1];o.selectionStart!==0&&x!=null&&!x.match(/\s/)&&(f=` ${f}`)}g=Ke(o,f,b,e.multiline);let y=o.selectionStart,k=o.selectionEnd,A=l&&l.length>0&&b.indexOf(l)>-1&&g.length>0;if(p){let x=J(o);t=x.newlinesToAppend,n=x.newlinesToPrepend,f=t+i,b+=n}if(g.startsWith(f)&&g.endsWith(b)){let x=g.slice(f.length,g.length-b.length);if(u===m){let C=u-f.length;C=Math.max(C,y),C=Math.min(C,y+x.length),y=k=C}else k=y+x.length;return{text:x,selectionStart:y,selectionEnd:k}}else if(A)if(d&&d.length>0&&g.match(d)){b=b.replace(l,g);let x=f+b;return y=k=y+f.length,{text:x,selectionStart:y,selectionEnd:k}}else{let x=f+g+b;return y=y+f.length+g.length+b.indexOf(l),k=y+l.length,{text:x,selectionStart:y,selectionEnd:k}}else{let x=f+g+b;y=u+f.length,k=m+f.length;let C=g.match(/^\s*|\s*$/g);if(h&&C){let G=C[0]||"",X=C[1]||"";x=G+f+g.trim()+b+X,y+=G.length,k-=X.length}return{text:x,selectionStart:y,selectionEnd:k}}}function ae(o,e){let{prefix:t,suffix:n,surroundWithNewlines:i}=e,r=o.value.slice(o.selectionStart,o.selectionEnd),s=o.selectionStart,a=o.selectionEnd,l=r.split(`
