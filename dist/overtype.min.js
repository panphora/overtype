...
${a}`:r;if(d){let k=t.value[t.selectionStart-1];t.selectionStart!==0&&k!=null&&!k.match(/\s/)&&(g=` ${g}`)}m=Ln(t,g,v,e.multiline);let w=t.selectionStart,b=t.selectionEnd,L=c&&c.length>0&&v.indexOf(c)>-1&&m.length>0;if(p){let k=$e(t);n=k.newlinesToAppend,o=k.newlinesToPrepend,g=n+i,v+=o}if(m.startsWith(g)&&m.endsWith(v)){let k=m.slice(g.length,m.length-v.length);if(u===f){let E=u-g.length;E=Math.max(E,w),E=Math.min(E,w+k.length),w=b=E}else b=w+k.length;return{text:k,selectionStart:w,selectionEnd:b}}else if(L)if(l&&l.length>0&&m.match(l)){v=v.replace(c,m);let k=g+v;return w=b=w+g.length,{text:k,selectionStart:w,selectionEnd:b}}else{let k=g+m+v;return w=w+g.length+m.length+v.indexOf(c),b=w+c.length,{text:k,selectionStart:w,selectionEnd:b}}else{let k=g+m+v;w=u+g.length,b=f+g.length;let E=m.match(/^\s*|\s*$/g);if(h&&E){let me=E[0]||"",Y=E[1]||"";k=me+g+m.trim()+v+Y,w+=me.length,b-=Y.length}return{text:k,selectionStart:w,selectionEnd:b}}}function nt(t,e){let{prefix:n,suffix:o,surroundWithNewlines:i}=e,r=t.value.slice(t.selectionStart,t.selectionEnd),s=t.selectionStart,a=t.selectionEnd,c=r.split(`
...
`),processed:o}}function se(t,e){return e?"- ":`${t+1}. `}function Sn(t,e){let n,o,i;return t.orderedList?(n=Xe(e),o=Ye(n.text),i=o.text):(n=Ye(e),o=Xe(n.text),i=o.text),[n,o,i]}function En(t,e){let n=t.selectionStart===t.selectionEnd,o=t.selectionStart,i=t.selectionEnd;tt(t);let r=t.value.slice(t.selectionStart,t.selectionEnd),[s,a,c]=Sn(e,r),d=c.split(`
...
</svg>`,on=`<svg viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
