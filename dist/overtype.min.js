...
`)}function Q(o,e={}){return{...o,colors:{...o.colors,...e}}}function te(o={}){let{fontSize:e="14px",lineHeight:t=1.6,fontFamily:n='"SF Mono", SFMono-Regular, Menlo, Monaco, "Cascadia Code", Consolas, "Roboto Mono", "Noto Sans Mono", "Droid Sans Mono", "Ubuntu Mono", "DejaVu Sans Mono", "Liberation Mono", "Courier New", Courier, monospace',padding:i="20px",theme:r=null,mobile:s={}}=o,a=Object.keys(s).length>0?`
...
`;)d++;o.selectionStart=a,o.selectionEnd=d}else ce(o);let p=e(o);if(t.adjustSelection){let l=o.value.slice(o.selectionStart,o.selectionEnd).startsWith(t.prefix),c=t.adjustSelection(l,n,i,a);p.selectionStart=c.start,p.selectionEnd=c.end}else if(t.prefix){let l=o.value.slice(o.selectionStart,o.selectionEnd).startsWith(t.prefix);r?l?(p.selectionStart=Math.max(n-t.prefix.length,a),p.selectionEnd=p.selectionStart):(p.selectionStart=n+t.prefix.length,p.selectionEnd=p.selectionStart):l?(p.selectionStart=Math.max(n-t.prefix.length,a),p.selectionEnd=Math.max(i-t.prefix.length,a)):(p.selectionStart=n+t.prefix.length,p.selectionEnd=i+t.prefix.length)}return p}function U(o,e){let t,n,{prefix:i,suffix:r,blockPrefix:s,blockSuffix:a,replaceNext:p,prefixSpace:d,scanFor:l,surroundWithNewlines:c,trimFirst:h}=e,u=o.selectionStart,v=o.selectionEnd,f=o.value.slice(o.selectionStart,o.selectionEnd),g=re(f)&&s&&s.length>0?`${s}
`:i,y=re(f)&&a&&a.length>0?`
${a}`:r;if(d){let L=o.value[o.selectionStart-1];o.selectionStart!==0&&L!=null&&!L.match(/\s/)&&(g=` ${g}`)}f=Ye(o,g,y,e.multiline);let w=o.selectionStart,x=o.selectionEnd,P=p&&p.length>0&&y.indexOf(p)>-1&&f.length>0;if(c){let L=K(o);t=L.newlinesToAppend,n=L.newlinesToPrepend,g=t+i,y+=n}if(f.startsWith(g)&&f.endsWith(y)){let L=f.slice(g.length,f.length-y.length);if(u===v){let C=u-g.length;C=Math.max(C,w),C=Math.min(C,w+L.length),w=x=C}else x=w+L.length;return{text:L,selectionStart:w,selectionEnd:x}}else if(P)if(l&&l.length>0&&f.match(l)){y=y.replace(p,f);let L=g+y;return w=x=w+g.length,{text:L,selectionStart:w,selectionEnd:x}}else{let L=g+f+y;return w=w+g.length+f.length+y.indexOf(p),x=w+p.length,{text:L,selectionStart:w,selectionEnd:x}}else{let L=g+f+y;w=u+g.length,x=v+g.length;let C=f.match(/^\s*|\s*$/g);if(h&&C){let J=C[0]||"",X=C[1]||"";L=J+g+f.trim()+y+X,w+=J.length,x-=X.length}return{text:L,selectionStart:w,selectionEnd:x}}}function pe(o,e){let{prefix:t,suffix:n,surroundWithNewlines:i}=e,r=o.value.slice(o.selectionStart,o.selectionEnd),s=o.selectionStart,a=o.selectionEnd,p=r.split(`
